...
// 文章类型 → 中文标签(面包屑末级 / 集群导航)。见《网站结构.md》§7。
...
/** 从 glob 集合 id(如 'cron/what-is-cron')取末段作为 URL slug */
...
/**
 * 取某工具集群下的全部内容页(按 order 排序),供支柱页"深入了解"与内容页兄弟模块共用。
 * 单一数据源 = content collection,新增文章零改动自动出现在内链里。见《内链策略.md》§5.3。
 */
export async function getClusterArticles(tool: string): Promise<ClusterArticle[]> {
